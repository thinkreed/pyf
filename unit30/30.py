...
connect_pattern = 'is connected to'
like_pattern = 'likes to play'
...
def create_data_structure(string_input):
    network = {}
    for sentence in string_input.split('.'):
        user_name = sentence[:sentence.find(' ')]
        index_of_like = sentence.find(like_pattern)
        index_of_connection = sentence.find(connect_pattern)
        if not user_name or (index_of_like == -1 and index_of_connection == -1):
            continue
...
def create_empty_user_information_dictionary():
    return {key_connections: [], key_likes: []}
